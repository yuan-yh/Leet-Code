...
情况：when first join, the onboarding task was在前端团队实习期间，我被分配为我们内部的一个工具构建一个标签页导航系统——类似于浏览器标签的工作原理。我当时对代码库和团队还比较新。
...
情况：我们团队正在为大学橄榄球运动员打造一个交易平台——就像体育赛事的股票市场。由于许可问题，我们正在转向新的外部数据源，而我负责构建数据摄取的流程。一个关键环节是玩家身份匹配系统——将外部玩家ID转换为内部UUID，使平台其他部分无需知道我们使用的是哪个数据源。
任务：匹配流水线有三层备份：首先尝试精确源ID查找，然后基于URL匹配，最后是规范化名称匹配。问题是当三层都失败时该怎么办。前任开发者开始构建一套模糊匹配系统，包含置信度评分、人工审核队列和审批流程，团队成员必须检查每个模糊案例后才能接收记录。这已经开发了几周，但仍未完成——而且没有针对未匹配玩家的解决方案路径，流程无法发布。
动作场面：在继续设计之前，我决定先看看实际数据。我用完整数据集对比了前两个层次，发现绝大多数玩家仅通过源ID或网址匹配成功。名字匹配的层级捕捉到了大部分剩余的。“三种都失败”的情况大约是每次服用几千例中的几例。
所以我提议：完全跳过模糊匹配系统。如果三层都失败，记录完整上下文——外部ID、名称、源URL、尝试过的层级——并作为新记录插入播放器。我的想法是，重复的既便宜又可逆。你合并两条记录后一切就没问题了。但错误的合并——将两个不同的人合并成一个身份——会扭曲历史数据，且极其难以理清。所以安全的默认做法是把它们分开。
我添加了一个轻量级监控警报，在团队频道中标记这些案件，方便有人定期查看。但没有阻塞的工作流程，没有审批门槛，也没有复杂的评分逻辑。
结果：实现过程大约只用了一天，而之前在模糊匹配上花了几周时间。管道按计划发货。实际上，未匹配病例很少见——每周只有几例——团队在例行检查中几分钟内就能解决。我们从未遇到过因错误合并而导致的数据损坏问题，这正是设计复杂设计最初的担忧。
学习：这让我学会了在设计解决方案之前，要先量化问题。模糊匹配系统正在构建，以应对最终证明的极小边缘情况，而简单路径——偶尔出现重复——的成本远低于复杂路径：运输延迟、维护更多代码以及可能出错的更多问题。从那以后，我的默认做法是先问“这种情况到底发生过多少次？如果我走简单路线，最坏的情况会怎样？”然后再决定是否采用复杂的设计。

### 9 | **Obstacle / Unexpected Changes** — Obstacles in a project, unexpected pivot, or change of direction mid-project | 4 |
情况：实习期间，我参与了内部账单管理工具的发票编辑功能开发。财务团队每天处理大量发票，我注意到许多发票遵循类似模式——相同的取货地点、相似的日期范围、向同一家饮料公司收费。于是我提议添加一个批量编辑功能，并得到了导师的批准，开始构建它。
任务：我当时已经处于实施中期，同事提出了一个严重的担忧，实际上阻碍了我当前的设计。她认为批处理操作可能会破坏数据库——无论是意外错误还是一次性清除记录的恶意行为。因为她对系统历史有更多了解，这件事我不能轻易否定。我需要找到一种方法来回应她的担忧而不削弱这个功能，或者干脆放弃这种做法。
动作场面：我没有变得防御或放弃这个想法，而是暂停了实施，把她的担忧当作新的设计限制。首先，我提取了发票数据，确认大约70%的编辑遵循相同的模式——相同的制造商、地点和账单方——这告诉我，如果能解决安全问题，这个功能仍然值得继续尝试。其次，我和导师讨论了保障策略。在研究了客户的工作流程和界面布局后，我决定对原设计做两项修改：将可批量编辑的发票数量限制在10张，并实现同步日志，以便任何批处理操作都能回滚。第三，我把修改后的提案带回给同事。我特别向她讲解了编辑上限和回滚机制，展示了他们如何解决她最初的担忧。她同意更新后的计划很稳妥，我们一起前进。
结果：尽管中途切换，批量编辑功能还是在冲刺中发布。账单团队给予了积极反馈——他们的工作流程明显加快了。发布后没有发生数据事件，安全措施也按预期运作。
学习：我了解到项目中遇到的意外障碍不一定意味着必须重新开始。通过停下来理解这个问题，而不是情绪化反应，我能够把它当作设计上的限制，而不是死胡同。最终的功能实际上比我最初的提案更强——回滚机制和编辑上限根本不在我最初的设计中。今后，当有人提出阻挡问题时，我的第一反应是问“我该如何围绕这个问题设计？”而不是“我该如何争论过去？”
